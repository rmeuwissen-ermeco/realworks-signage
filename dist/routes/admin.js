...
router.post("/tenants/create", async (req, res) => {
    ...
    let finalSlug = slug;
    let n = 2;
    while (await prisma_1.prisma.tenant.findUnique({ where: { slug: finalSlug } })) {
        finalSlug = `${slug}-${n++}`;
    }
...
router.post("/tenants/:tenantId/streams/create", async (req, res) => {
    ...
    let n = 2;
    while (await prisma_1.prisma.stream.findUnique({ where: { streamKey } })) {
        streamKey = `${baseKey}-${n++}`.slice(0, 40);
    }
    await prisma_1.prisma.stream.create({
        data: {
            tenantId,
            title,
            streamKey,
...
router.post("/streams/:streamId/publish", async (req, res) => {
    ...
    const stream = await prisma_1.prisma.stream.update({
        where: { id: streamId },
        data: {
